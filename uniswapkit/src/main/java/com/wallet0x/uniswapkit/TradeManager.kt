...
import com.wallet0x.uniswapkit.contract.*
import com.wallet0x.uniswapkit.models.*
import com.wallet0x.uniswapkit.models.Token.Erc20
import com.wallet0x.uniswapkit.models.Token.Ether
...
class TradeManager(
    ...
    fun pair(tokenA: Token, tokenB: Token): Single<Pair> {
        ...
        return evmKit.call(pairAddress, com.wallet0x.uniswapkit.contract.GetReservesMethod().encodedABI())
                .map { data ->
    ...
    private fun buildMethodForExactOut(tokenIn: Token, tokenOut: Token, path: List<Address>, to: Address, deadline: BigInteger, tradeData: TradeData, trade: Trade): ContractMethod {
        ...
        return when {
            tokenIn is Ether && tokenOut is Erc20 -> com.wallet0x.uniswapkit.contract.SwapETHForExactTokensMethod(
                amountOut,
                path,
                to,
                deadline
            )
            tokenIn is Erc20 && tokenOut is Ether -> com.wallet0x.uniswapkit.contract.SwapTokensForExactETHMethod(
                amountOut,
                amountInMax,
                path,
                to,
            ...
            tokenIn is Erc20 && tokenOut is Erc20 -> com.wallet0x.uniswapkit.contract.SwapTokensForExactTokensMethod(
                amountOut,
                amountInMax,
                path,
                to,
    ...
    private fun buildMethodForExactIn(tokenIn: Token, tokenOut: Token, path: List<Address>, to: Address, deadline: BigInteger, tradeData: TradeData, trade: Trade): ContractMethod {
        ...
        return when {
            tokenIn is Ether && tokenOut is Erc20 -> com.wallet0x.uniswapkit.contract.SwapExactETHForTokensMethod(
                amountOutMin,
                path,
                to,
                deadline
            )
            tokenIn is Erc20 && tokenOut is Ether -> com.wallet0x.uniswapkit.contract.SwapExactTokensForETHMethod(
                amountIn,
                amountOutMin,
                path,
                to,
            ...
            tokenIn is Erc20 && tokenOut is Erc20 -> com.wallet0x.uniswapkit.contract.SwapExactTokensForTokensMethod(
                amountIn,
                amountOutMin,
                path,
                to,
