...
import com.wallet0x.ethereumkit.api.jsonrpc.SubscribeJsonRpc
import com.wallet0x.ethereumkit.api.jsonrpcsubscription.NewHeadsRpcSubscription
import com.wallet0x.ethereumkit.api.jsonrpcsubscription.RpcSubscription
    ...
    override fun <T> single(rpc: JsonRpc<T>): Single<T> {
        return Single.create { emitter ->
            send(
                    rpc = rpc,
                    onSuccess = {
                        emitter.onSuccess(it!!)  // ✅ Kotlin 1.9.23 compatibility
                    },
                    onError = {
                        emitter.onError(it)
                    }
